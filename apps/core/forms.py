...
class GroupsForm(forms.Form):
    group = forms.ModelChoiceField(queryset=GroupTemplate.objects.filter().order_by("name"), empty_label=None)
    content = forms.CharField(widget=forms.Textarea(attrs={'class': 'form-control', 'rows': "30", 'cols': "100"}))
        ...
        for testname in rows.split():
            try:
                ...
                if test.name not in tests_exists:
                    gtt = GroupTestTemplate(test=test, group=group, priority=priority)
                    gtt.save()
                    priority += 1
...
class JobForm(forms.Form):
    jobs1=forms.ModelChoiceField(queryset=JobTemplate.objects.filter(is_enable=True).order_by("whiteboard"), empty_label=None)
    jobs2=forms.ModelChoiceField(queryset=JobTemplate.objects.filter(is_enable=True).order_by("whiteboard"), empty_label=None)
