...
function formateTaskList(tasks) {
	...
	$.each(tasks, function(i, task){
    	...
    	if (task.statusbyuser == 11 || task.statusbyuser == 'waived') tres=11;
...
function loadTasks(uid, from, filter) {
	...
	var errors = '';
	if (filter == 'errors') errors = '&results=3,4,6'; // WARN, FAIL, NEW
	$.getJSON('/api/v1/task/?recipe__uid='+uid+errors+'&limit=500&ordering=uid', function(data){
		var tabC = $("div[data-name='R:"+uid+"']", detailPanel.elm);
		$(".tasks-list tbody", tabC).html(formateTaskList(data.results));
		$(".tasksLabel", tabC).html('Tasks ('+data.count+')');
		$(".tasks-list table", tabC).attr('data-counttasks', data.count);
...
function previewRecipe(id) {
    ...
    lastID = id
    if(id) {
        $.getJSON("/api/v1/recipe/" + lastID + "/", function(data) {
			var loadToBackend = openUIDs.has(data.id);
			if (lastID != data.id && !loadToBackend) { return; }
			if (loadToBackend) {
				openUIDs.del(data.id);
				$('.dashboard tbody td[id="R:'+data.uid+'"]').addClass('selected');
			} else {
				$("td.hover").removeClass('hover');
				$('.dashboard tbody td[id="R:'+data.uid+'"]').addClass('hover');
			}
			loadTasks(data.uid, 0, 'errors')
			...
			if (data.tasks) {
				var lastStyle = "";
				var step =  100 / data.tasks.length;
	            for(ix in data.tasks) {
					var style = '';
					var task = data.tasks[ix];
					if (task.result == 5) { //'pass'
						style = 'success';
					} else if (task.result == 3) {
						style = 'warning';
					} else if (task.result == 4 && task.statusbyuser != 11) {
						style = 'danger';
					} else if (task.statusbyuser == 11) {
						style = 'info';
					} else {
						continue;
					}
					message += '<div class="progress-bar progress-bar-'+style+'" role="progressbar" '+
					           'style="width: '+step+'%" title="'+style+' tasks ('+1+')" >'+
							   '</div>';
            	}
			}
            message += '</div>';
            ...
            if (data.comments.length > 0) {
				coment = '<span class="glyphicon glyphicon-list-alt" title="This job has got '+data.comments.length+' comments."></span>';
            }

            message += '<strong class="tasksLabel">Tasks () '+coment+'</strong>';
...
function saveRecipePanel() {
	...
	previewRecipe($(this).attr('data-id'));
}
